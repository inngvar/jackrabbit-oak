...
public class DataStoreCacheUpgradeUtils {
    ...
    public static void moveDownloadCache(final File path) {
        ...
        final Spliterator<File> spliterator = Files.fileTraverser().depthFirstPostOrder(Arrays.asList(path.listFiles())).spliterator();
        Iterator<File> iterator =
                StreamSupport.stream(spliterator,false)
                .filter((Predicate<File>) input -> input.isFile()
                    && !input.getParentFile().equals(path)
                    && !notInExceptions(input, exceptions)).iterator();
